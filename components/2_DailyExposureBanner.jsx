...
const Element = ({ name, src }) => (
    <div className='group rounded-2xl border border-amber-200/20 bg-white/[0.04] p-4 shadow-[0_8px_28px_-20px_rgba(251,191,36,0.95)] backdrop-blur transition-all duration-300 hover:-translate-y-1 hover:border-amber-300/60 hover:bg-white/[0.08]'>
        ...
        <p className='pt-3 text-center text-base font-medium capitalize text-white md:text-lg'>{name}</p>
    </div>
);

const DailyExposureBanner = () => {
    return (
        <section className='relative overflow-hidden bg-[#060606] px-5 py-20'>
            ...
            <div className='relative mx-auto max-w-6xl'>
                <div className='mx-auto max-w-6xl rounded-3xl border border-white/10 bg-gradient-to-b from-white/[0.09] via-white/[0.04] to-transparent px-6 py-10 text-white md:px-10 md:py-10'>
                    ...
                    <h1 className='mt-3 text-center text-3xl font-semibold leading-tight md:text-5xl'>
                        emGuarde neutralises EMF and RF radiation emitted by your devices
                    </h1>
                    <p className='mx-auto mt-8 max-w-3xl text-center text-lg text-white/85 md:text-lg'>
                ...
                <div className='mx-auto mt-5 grid max-w-5xl grid-cols-2 gap-4 md:grid-cols-3 md:gap-5'>
                    {list.map((e) => (
                        <Element {...e} key={e.name} />
                    ))}
                </div>

                <div className='mx-auto mt-5 max-w-5xl rounded-2xl border border-white/10 bg-white/[0.04] p-5 text-white text-center md:px-8'>
                    <p className='text-lg text-amber-100'>
                        These devices and appliances above are among the{' '}
                        <span className='font-semibold text-amber-300'>
...
export default DailyExposureBanner;
